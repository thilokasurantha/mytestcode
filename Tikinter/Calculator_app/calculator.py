...
class CalculatorApp :
    ...
    def make_gui(self) :
        ...
        self.eqalence = Button(self.frame5, text="=", width=7, height=5, font=("Source Sans Pro", 12), command=self.calculations)
    ...
    def calculations(self) :
        self.get = self.show_entry.get()
        self.split = self.get.split()
        print(self.split)
        self.calculating = {
        "+" : int(self.split[0]) + int(self.split[2]),
        "*" : int(self.split[0]) * int(self.split[2]),
        "/" : int(self.split[0]) // int(self.split[2]),
        "-" : int(self.split[0]) - int(self.split[2])
        }
